...
def get_tensor_from_img(img):
    return tf.convert_to_tensor(
        tf.keras.preprocessing.image.img_to_array(img), dtype=tf.float32
    )
...
def has_black_background(rgb_img):
    ...
    together = tf.reshape(
        tf.concat([top_rows, bottom_rows, left_cols, right_cols], axis=0), [-1]
    ).numpy()
    ...
    for count, filepath in enumerate(filepaths):
        if filepath.exists() and filepath.is_file():
            ...
            if not destination_path.exists():
                ...
                while (
                    destination_dir / pathlib.Path(dpath_stem + str(num) + dpath_suffix)
                ).exists():
                    num += 1
                ...
                shutil.copy2(
                    filepath,
                    destination_dir
                    / pathlib.Path(dpath_stem + str(num) + dpath_suffix),
                )
        else:
...
def main():
    ...
    black_bkgd_pokes = [
        p
        for p in pokes
        if has_black_background(get_tensor_from_img(get_rgb_img_from_path(p)))
    ]
    white_bkgd_pokes = [
        p
        for p in pokes
        if not has_black_background(get_tensor_from_img(get_rgb_img_from_path(p)))
    ]
    ...
    convertible_white_bkgd = [
        p for p in white_bkgd_pokes if PIL.Image.open(p).mode == "RGBA"
    ]
